        ...
        public async Task<ResIngrediente> IngresarIngrediente(int idCategoriaIngrediente, string nombreIngrediente, string descripcion, string urlImagen, decimal precio)
        {
            ...
            try
            {
                ...
                if (res.ListaDeErrores.Count() == 0)
                {
                    ...
                    {
                        var response = await httpClient.PostAsync("https://apibubblehouse.azurewebsites.net/api/ingrediente/ingresar", jsonContent);
        ...
        public async Task<ResIngrediente> ActualizarIngrediente(int idIngrediente, int idCategoriaIngrediente, string nombreIngrediente, string descripcion, string urlImagen, decimal precio)
        {
            ...
            try
            {
                ...
                if (res.ListaDeErrores.Count() == 0)
                {
                    ...
                    using (HttpClient httpClient = new HttpClient())
                    {
                        var response = await httpClient.PutAsync("https://apibubblehouse.azurewebsites.net/api/ingrediente/modificar", jsonContent);
                        if (response.IsSuccessStatusCode)
                        {
                            var responseContent = await response.Content.ReadAsStringAsync();
                            res = JsonConvert.DeserializeObject<ResIngrediente>(responseContent);
                ...
                if (id != 0)
                {
                    ...
                    using (HttpClient httpClient = new HttpClient())
                    {
                        ...
                        var request = new HttpRequestMessage
                        {
                            ...
                            RequestUri = new Uri("https://apibubblehouse.azurewebsites.net/api/ingrediente/eliminar"),
                            Content = jsonContent
                        };
